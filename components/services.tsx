    ...
    <section id="services" className="py-32 px-6 bg-card/30 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20 animate-fade-in-up">
          <p className="text-sm tracking-widest text-amber-600 mb-4 font-medium animate-fade-in-up delay-300">OUR EXPERTISE</p>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {services.map((service, index) => {
            ...
            return (
              <Card
                key={service.id}
                className={`p-12 bg-card/50 backdrop-blur-sm border-border hover:border-amber-500/50 transition-all duration-700 group hover:shadow-2xl hover:shadow-amber-500/20 hover:scale-105 animate-fade-in-up delay-${(index + 1) * 200} hover:animate-glow`}
              >
                <div className="flex items-start gap-6">
                  <div className="text-7xl font-serif font-bold bg-gradient-to-br from-muted-foreground/20 to-amber-600/40 bg-clip-text text-transparent group-hover:from-amber-600/60 group-hover:to-amber-500/50 transition-all duration-700 group-hover:scale-110">
                    {String(service.order_index).padStart(2, '0')}
                  </div>
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center gap-3">
                      <IconComponent className="h-6 w-6 text-amber-600 group-hover:text-amber-500 group-hover:scale-110 transition-all duration-500" />
                      <h3 className="text-2xl font-serif font-semibold tracking-wide text-foreground group-hover:text-amber-600 transition-colors duration-500">{service.title}</h3>
                    </div>
                    <p className="text-foreground/70 leading-relaxed text-base group-hover:text-foreground/90 transition-colors duration-500">{service.description}</p>
                  </div>
                </div>
